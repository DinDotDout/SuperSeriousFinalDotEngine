...
#define HM_FN(name) DOT_CONCAT(HM_NAME, _##name)

typedef struct HM_NAME{
    void  **node_buckets; // HM_NODE
    u32     bucket_count;
    u32     elem_count;
}HM_NAME;
...
internal HM_T  *HM_FN(push_new)(Arena *arena, HM_NAME *hashmap, HM_K key, u64 hash);
...
{
    DOT_ASSERT(IS_POW2(bucket_count));
    hashmap->bucket_count = bucket_count;
    hashmap->node_buckets = (void **)PUSH_ARRAY(arena, HM_NODE*, hashmap->bucket_count);
}
...
internal HM_T *
HM_FN(push_new)(Arena *arena, HM_NAME *hashmap, HM_K key, u64 hash)
{
    ...
    HM_NODE **bucket        = (HM_NODE**)&hashmap->node_buckets[shader_bucket_idx];
...
HM_FN(get_or_create)(Arena *arena, HM_NAME *hashmap, HM_K key)
{
    ...
    if(!elem){
        elem = HM_FN(push_new)(arena, hashmap, key, hash);
    }
    return(elem);
}

// (jd) NOTE: Pass in a HashMap_Iter = {0}
internal HM_T *
