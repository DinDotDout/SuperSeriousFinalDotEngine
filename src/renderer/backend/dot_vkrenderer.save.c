...
internal inline DOT_CandidateDeviceInfo DOT_RendererBackendVk_PickBestDevice(VkInstance instance, VkSurfaceKHR surface) {
    TempArena temp = Memory_GetScratch();
    u32 device_count = 0;
    ...
    for (u32 i = 0; i < device_count; i++) {
        ...
        array(VkQueueFamilyProperties) qprops = PushArray(temp.arena, VkQueueFamilyProperties, qcount);
...
internal inline bool DOT_VK_AllLayers(){
    TempArena temp = Memory_GetScratch();
    u32 layer_count = 0;
...
internal void DOT_RendererBackendVk_InitVulkan(DOT_RendererBackendBase* ctx, DOT_Window* window) {
    ...
    TempArena temp = Memory_GetScratch(0);
    if (!DOT_VK_AllLayers()){
    ...
    // Device creation
    {
        ...
        DOT_CandidateDeviceInfo candidate_device_info = DOT_RendererBackendVk_PickBestDevice(renderer_ctx->instance, renderer_ctx->surface);
        ...
        // capabilities
        {
            ...
            // u32 format_count;
            // array(VkSurfaceFormat2KHR) surface_formats = PushArray(temp.arena, VkSurfaceFormat2KHR, formats.format_count);
            ...
            vkGetPhysicalDeviceSurfaceFormats2KHR(renderer_ctx->device.gpu, &surface_info, &formats.format_count, NULL);
            if (formats.format_count == 0){
                DOT_ERROR("No valid formats found");
            }
